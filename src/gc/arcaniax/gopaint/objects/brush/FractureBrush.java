		...
		for (Block b : blocks){
					if ((!pb.isSurfaceModeEnabled())||gc.arcaniax.gopaint.utils.Surface.isOnSurface(b.getLocation(), p.getLocation())){
						if ((!pb.isMaskEnabled())||(b.getType().equals(pb.getMask().getMaterial())&&(XMaterial.isNewVersion()||b.getData()==pb.getMask().getData()))){
							if (gc.arcaniax.gopaint.utils.Height.getAverageHeightDiffFracture(b.getLocation(), gc.arcaniax.gopaint.utils.Height.getHeight(p, b.getLocation()), 1, p)>=0.1){
								if (gc.arcaniax.gopaint.utils.Height.getAverageHeightDiffFracture(b.getLocation(), gc.arcaniax.gopaint.utils.Height.getHeight(p, b.getLocation()), pb.getFractureDistance(), p)>=0.1){
									Random r = new Random();
		...
		for (Block b : blocks){
					if ((!epb.isSurfaceModeEnabled())||gc.arcaniax.gopaint.utils.Surface.isOnSurface(b.getLocation(), p.getLocation())){
						if ((!epb.isMaskEnabled())||(b.getType().equals(epb.getMask().getMaterial())&&(XMaterial.isNewVersion()||b.getData()==epb.getMask().getData()))){
							if (gc.arcaniax.gopaint.utils.Height.getAverageHeightDiffFracture(b.getLocation(), gc.arcaniax.gopaint.utils.Height.getHeight(p, b.getLocation()), 1, p)>=0.1){
								if (gc.arcaniax.gopaint.utils.Height.getAverageHeightDiffFracture(b.getLocation(), gc.arcaniax.gopaint.utils.Height.getHeight(p, b.getLocation()), epb.getFractureDistance(), p)>=0.1){
									Random r = new Random();

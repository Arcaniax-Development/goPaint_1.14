...
import gc.arcaniax.gopaint.utils.BlockUtils;
import gc.arcaniax.gopaint.utils.XMaterial;
import gc.arcaniax.gopaint.utils.curve.BezierSpline;
...
public class PaintBrush extends Brush{
    private static HashMap<String, List<Location>> selectedPoints = new HashMap<>();
    ...
    public void paint(Location loc, Player p) {
        ...
        if (!selectedPoints.containsKey(p.getName())){
            ...
            List<Block> blocks = gc.arcaniax.gopaint.utils.Sphere.getBlocksInRadiusWithAir(locs.get(0), size);
            List<BlockPlace> placedBlocks = new ArrayList<BlockPlace>();
            for (Block b : blocks){
                Random r = new Random();
                int random = r.nextInt(pbBlocks.size());
                double rate = (b.getLocation().distance(locs.get(0)) - ((double)size/2.0)*((100.0-(double)falloff)/100.0))/(((double)size/2.0) - ((double)size/2.0)*((100.0-(double)falloff)/100.0));
                if (!(r.nextDouble()<=rate)){
                    ...
                    for (int y = 0; y<=maxCount; y++){
                        ...
                        if (BlockUtils.isLoaded(location)&&(!location.getBlock().getType().equals(XMaterial.AIR.parseMaterial()))){
                            if ((!pb.isSurfaceModeEnabled())||gc.arcaniax.gopaint.utils.Surface.isOnSurface(location, p.getLocation())){
                                if ((!pb.isMaskEnabled())||(b.getType().equals(pb.getMask().getMaterial())&&(XMaterial.isNewVersion()||b.getData()==pb.getMask().getData()))){
                                    placedBlocks.add(new BlockPlace(location,
            ...
            for (Block b : blocks){
                ...
                if (!(r.nextDouble()<=rate)){
                    ...
                    for (int y = 0; y<=maxCount; y++){
                        ...
                        Location location = new Location(l.getWorld(), l.getBlockX(), l.getBlockY(), l.getBlockZ());
                        if (BlockUtils.isLoaded(location)&&(!location.getBlock().getType().equals(XMaterial.AIR.parseMaterial()))){
                            if ((!epb.isSurfaceModeEnabled())||gc.arcaniax.gopaint.utils.Surface.isOnSurface(location, p.getLocation())){
